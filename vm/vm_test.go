...
func TestFunctionsWithReturnValue(t *testing.T) {
	tests := []vmTestCase{
		{
			input: `
let noReturn = fn() {};
...
func TestCallingFunctionsWithWrongArguments(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if err.Error() != tt.expected {
			t.Fatalf("wrong WM error: want=%q,got=%q", tt.expected, err)
		}
	}
}
...
func TestBuiltinFunctions(t *testing.T) {
	tests := []vmTestCase{
		...
		{`last([1,2,3])`, 3},
		{`last([])`, Null},
		{`last(1)`, &object.Error{Message: "argument to " +
			"`last` must be ARRAY,got INTEGER"}},
		{`rest([1,2,3])`, []int{2, 3}},
		{`rest([])`, Null},
		{`push([],1)`, []int{1}},
